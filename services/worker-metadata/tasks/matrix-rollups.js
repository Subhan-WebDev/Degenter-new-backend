  ...
  await DB.query(`
    ...
    enriched AS (
      SELECT
        ...
        CASE WHEN p.is_uzig_quote THEN q.buy_quote_base  / 1e6
             ELSE q.buy_quote_base  / power(10::numeric, COALESCE(qtk.exponent::int,6)) END AS vol_buy_quote,
        CASE WHEN p.is_uzig_quote THEN q.sell_quote_base / 1e6
             ELSE q.sell_quote_base / power(10::numeric, COALESCE(qtk.exponent::int,6)) END AS vol_sell_quote,
        CASE WHEN p.is_uzig_quote THEN (q.buy_quote_base  / 1e6)
             ELSE ((q.buy_quote_base  / power(10::numeric, COALESCE(qtk.exponent::int,6))) * COALESCE(pr.price_in_zig,0)) END AS vol_buy_zig,
        CASE WHEN p.is_uzig_quote THEN (q.sell_quote_base / 1e6)
             ELSE ((q.sell_quote_base / power(10::numeric, COALESCE(qtk.exponent::int,6))) * COALESCE(pr.price_in_zig,0)) END AS vol_sell_zig
      FROM q
  ...
  await DB.query(`
    ...
    reserves AS (
      SELECT
        ...
        CASE WHEN p.is_uzig_quote THEN (s.reserve_quote_base / 1e6)
             ELSE (s.reserve_quote_base / power(10::numeric, COALESCE(q.exponent::int,6))) END AS reserve_quote_disp
      FROM pool_state s
...
async function rollTokenMatrix(label, onlyTokenId = null) {
  await DB.query(`
    ...
    token_price_disp AS (
      SELECT t.token_id, t.exponent AS texp,
        CASE
    ...
    SELECT token_id, $1, price_in_zig,
...
async function start() {
  ...
  while (true) {
    try { await once(); } catch (e) { log.warn('[matrix]', e.message || e); }
...
export async function refreshTokenMatrixOnce(tokenId) {
  if (!tokenId) return;
  for (const [label] of BUCKETS) await rollTokenMatrix(label, tokenId);
