...
const ZIGSCAN_BASE_RAW = process.env.ZIGSCAN_BASE || 'https://zigchain-mainnet-api.wickhub.cc/';
...
async function getCreatorBalance(creatorAddr, denom) {
  const url =
    `${ZIGSCAN_BASE}/cosmos/bank/v1beta1/balances/${encodeURIComponent(creatorAddr)}` +
...
export async function scanTokenOnce(tokenId, denom) {
  ...
  try {
    ...
    const { top10Pct, holdersCount } = await top10ShareOfMax(tokenId, maxSupplyBase);
